    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Picker("Position", selection: $session.positionFilter) {
                        Text("All Positions").tag(Optional<Int>.none)
                        ForEach(1...5, id: \.self) { pos in
                            Text("Position \(pos)").tag(Optional(pos))
                        }
                    }
                } header: {
                    Text("Position Filter")
                } footer: {
                    Text("Drill a single box position or randomise across all five.")
                }
