...
final class TriadSession {
    ...
    /// Step 2: score the inversion answer. Returns true if correct.
    @discardableResult
    func answerInversion(_ inversion: TriadInversion) -> Bool {
        guard let q = currentQuestion else { return false }
        return inversion == q.shape.inversion
    }

    /// Step 3: score the root-note answer. Returns true if correct.
    /// Does NOT advance — the view handles feedback then calls advance().
    @discardableResult
